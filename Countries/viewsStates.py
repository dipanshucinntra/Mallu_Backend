...
##################################### OLD CODE REMOVE IF EVERY THING IS RUNNING GOOD ############
# #States Create API
# @api_view(["POST"])
# def create(request):
#     Code = request.data["Code"]
...
#                 print(st_data)
#                 print(json.dumps(st_data))
                
#                 res = requests.post(settings.BASEURL+'/States', data=json.dumps(st_data), cookies=r.cookies, verify=False)
...
#     id = request.data['id']
#     gst = request.data['GST']
#     st_obj = States.objects.filter(id=id)
#     print(len(st_obj))
#     if st_obj:
#         st = st_obj.first()
#         st.GST = gst
#         st.save()
#         st_json = StatesSerializer(st).data
#         return Response({"message": "Success", "status": 200,"data":[st_json]})
#     else:
#         return Response({"message": "States are not available in this Country","status": 201, "data":[]})
