...
from Service.models import DailyCustomerService
...
def dashboard(request):
    ...
    if "SalesEmployeeCode" in json_data:
        ...
        if json_data['SalesEmployeeCode'] != "":
            ...
            SalesEmployeeCode = showEmployeeData(SalesEmployeeCode)
            ...
            # bp_all = f"{str(SalesEmployeeCode)}"
            bp_all = BusinessPartner.objects.filter(SalesPersonCode__in=emp_ids).count()
            #print(bp_all)
...
def create(request):
    try:
        if Employee.objects.filter(Active="tYES").count() >= 75:
            return Response({"message": "User limit exceed more than 75", "status": 201, "data": []})
        elif request.data['userName']=='' or request.data['userName']== None :
            ...
            try:
                model = Employee(companyID=companyID, SalesEmployeeName=SalesEmployeeName, EmployeeID=EmployeeID, userName=userName, password=password, firstName=firstName, middleName=middleName,lastName=lastName, Email=Email, Mobile=Mobile, role=role, position=position, branch=branch, Active=Active, salesUnit=salesUnit, reportingTo=reportingTo, timestamp=timestamp, div=div, country = country, country_code = country_code, state = state,state_code = state_code, doj=doj)
                model.save()
...
def all_filter(request):
    ...
    if "SalesEmployeeCode" in json_data:
        ...
        if json_data['SalesEmployeeCode'] != "":
            ...
            SalesEmployeeCode = showEmployeeData(SalesEmployeeCode)
            # emp_obj = Employee.objects.get(SalesEmployeeCode=SalesEmployeeCode)
            # if emp_obj.role == 'admin' or emp_obj.role == 'ceo' or emp_obj.role == 'hr':
...
def all_filter_page(request):
    ...
    if "SalesEmployeeCode" in json_data:
        ...
        if json_data['SalesEmployeeCode'] != "":
            ...
            emps_all = Employee.objects.filter(Q(SalesEmployeeCode__in=SalesEmployeeCode,firstName__icontains=search, Active="tYES")|Q(SalesEmployeeCode__in=SalesEmployeeCode,lastName__icontains=search, Active="tYES")|Q(SalesEmployeeCode__in=SalesEmployeeCode, Email__icontains=search, Active="tYES")|Q(SalesEmployeeCode__in=SalesEmployeeCode, Mobile__icontains=search, Active="tYES")).order_by("-id")[startWith:endWith]
            emps_json = EmployeeSerializer(emps_all, many=True)
    ...
    if "SalesEmployeeCode" in json_data:
        if json_data['SalesEmployeeCode'] != "":
            ...
            if emp_obj.role == 'admin' or emp_obj.role == 'ceo':
                ...
                SalesEmployeeCode = [SalesEmployeeCode]
            emps_all = Employee.objects.filter(Q(SalesEmployeeCode__in=SalesEmployeeCode,firstName__icontains=search, Active="tYES")|Q(SalesEmployeeCode__in=SalesEmployeeCode,lastName__icontains=search, Active="tYES")|Q(SalesEmployeeCode__in=SalesEmployeeCode, Email__icontains=search, Active="tYES")|Q(SalesEmployeeCode__in=SalesEmployeeCode, Mobile__icontains=search, Active="tYES"))
            return Response({"message": "Success", "status": 200, "data": [{"total_count":emps_all}]})
            #return Response({"message": "Success","status": 201,"data":[{"emp":SalesEmployeeCode}]})
        else:
            return Response({"message": "Unsuccess", "status": 201, "data": [{"error": "SalesEmployeeCode?"}]})
    else:
...
def all_filter_reportingto(request):
    ...
    if "SalesEmployeeCode" in json_data:
        ...
        if json_data['SalesEmployeeCode'] != "":
            ...
            emps_all1 = []
            # .values('id', 'SalesEmployeeCode')
            emps_all = Employee.objects.filter(reportingTo=SalesEmployeeCode, Active="tYES").exclude(role="admin")
            print(len(emps_all))
            print(SalesEmployeeCode)
...
@api_view(["POST"])
def all_filter_reportingto_page(request):
    json_data = request.data
    PageNo = json_data['PageNo']
    try:
        MaxItem = json_data['MaxItem']
    ...
    if "SalesEmployeeCode" in json_data:
        ...
        if json_data['SalesEmployeeCode'] != "":
            ...
            emps_all1 = []
            # .values('id', 'SalesEmployeeCode')
            emps_all = Employee.objects.filter(reportingTo=SalesEmployeeCode)[startWith:endWith]
...
@api_view(["POST"])
def all_filter_assignto_page(request):
    json_data = request.data
    PageNo = json_data['PageNo']
    try:
        MaxItem = json_data['MaxItem']
    ...
    if "SalesEmployeeCode" in json_data:
        ...
        if json_data['SalesEmployeeCode'] != "":
            ...
            if len(emps_all) == 0:
                ...
                # .values('id', 'SalesEmployeeCode')
                emps_all = Employee.objects.filter(
                    reportingTo=emp_obj.reportingTo, Active="tYES")[startWith:endWith]
                emps_json = EmployeeSerializer(emps_all, many=True)
            else:
                #emps_json = EmployeeSerializer(emps_all, many=True)
                print("come")
                        ...
                        for all_tree in emps_all_tree:
                            ...
                            if len(emps_all_tree1) != 0:
                                ...
                                for all_tree1 in emps_all_tree1:
                                    ...
                                    emps_all1.append(all_tree1)
                            emps_all1.append(all_tree)
                data_obj = emps_all1[startWith:endWith]
                emps_json = EmployeeSerializer(data_obj, many=True)
            return Response({"message": "Success", "status": 200, "data": emps_json.data})
...
def one(request):
    try:
        ...
        if employee_obj.div != "":
            ...
            employee_json['div'] = []
        if SettingBranch.objects.filter(id=employee_obj.branch).exists():
            branch_obj = SettingBranch.objects.filter(id=employee_obj.branch).first()
            employee_json["branch_name"] = branch_obj.branch_name
        else:
            employee_json["branch_name"] = ""
...
def login(request):
    try:
        if BusinessPartner.objects.filter(Username=request.data["userName"], Is_Active="tYES").exists():
            userName = request.data["userName"]
            password = request.data["password"]
            FCM = request.data['FCM']
            print("userName : ", userName, "and : password:",password )  
            if BusinessPartner.objects.filter(Username=userName, Password=password).exists():
                employee_obj = BusinessPartner.objects.get(Username=userName, Password=password)
            else:
                print("incorrect")
                return Response({"message": "Username or password is incorrect", "status": 200, "data": [], "SAP": []})

            bp_data = BusinessPartnerSerializer(employee_obj).data
            bp_data["role"] = "customer"
            if Employee.objects.filter(SalesEmployeeCode=employee_obj.CreatedBy).exists():
                bp_emp = Employee.objects.filter(SalesEmployeeCode=employee_obj.CreatedBy).first()
                if Employee.objects.filter(SalesEmployeeCode=bp_emp.reportingTo).exists():
                    bp_report_name = Employee.objects.filter(SalesEmployeeCode=bp_emp.reportingTo).first()
                    bp_data["reporting_role"] = bp_report_name.role
                    bp_data["reporting_name"] = bp_report_name.SalesEmployeeName
                else:
                    bp_data["reporting_role"] = ""
                    bp_data["reporting_name"] = ""
            else:
                bp_data["reporting_role"] = ""
                bp_data["reporting_name"] = ""
            return Response({"message": "Success", "status": 200, "data": bp_data, "SAP": []})
        
        elif Employee.objects.filter(userName=request.data['userName'], Active="tYES").exists():
            userName = request.data['userName']
            password = request.data['password']
            FCM = request.data['FCM']
            print("userName : ", userName, "and : password:",password )    
            ...
            return Response({"message": "Email or Password is incorrect", "status": 201, "data": []})
    except Exception as e:
        return Response({"message": str(e), "status": "201", "data": [{"Error": str(e)}]})
        ...
        if request.data['id'] != "":
            ...
            #     return Response({"message": "All Qtr should be fill", "status": "201", "data": []})
                ...
                if len(request.data['monthly']) == 12:
                    ...
                    #     print("errorr monthly...!!")
                    #     return Response({"message": "Quaterley saved but monthly try again after fill all months", "status": "201", "data": []})
                    if q1!=(months[0]['amount'] + months[1]['amount'] + months[2]['amount']) or q2 != (months[3]['amount'] + months[4]['amount'] + months[5]['amount']) or q3!= (months[6]['amount'] + months[7]['amount'] + months[8]['amount']) or q4 != (months[9]['amount'] + months[10]['amount'] + months[11]['amount']):
                        print("errorr monthly...!!")
                        print(q1,q2,q3,q4)
                        return Response({"message": "Quaterley saved but monthly try again after fill all months", "status": "201", "data": []})
                    else:
...
def targetyr_all_filter(request):
    try:
        dt = request.data
        # emps = getAllReportingToUserId(dt['reportingTo'])
...
def ASE_list(request):
    try:
        ...
        emp_data = Employee.objects.filter(reportingTo=emp_id, Active="tYES", role="ASE")
        serializers = EmployeeSerializer(emp_data, many=True)
...
@api_view(["GET"])
# all_drop_asp
def all_drop_asp(request):
    try:
        url = settings.SERVICEURL+"/employee/all_drop_asp"
        emp_data = requests.get(url)
        if emp_data.status_code == 200:
            return Response({"message": "Success", "Status": 200, "data":emp_data.json()})
        else:
...
@api_view(["POST"])
# target visitor data
def target_visitor_count(request):
    try:
        Emp_id = request.data["Emp_id"]
        ...
        visit = 0
        left = 0
        emp_obj = Employee.objects.filter(SalesEmployeeCode=Emp_id).first()
        if emp_obj.role == "ASE":
            if filter_by == "Daily":
                # if DailyCustomerService.objects.filter(ase_id=Emp_id).exists():
                daily_data = DailyCustomerService.objects.filter(ase_id=Emp_id, create_date=tdate).count()
                target_data = TargetVisitor.objects.filter(assigned_to=Emp_id).first()
                total = int(target_data.daily_visit)
                visit = daily_data
                left  = total - visit
                if left < 0:
                    left = 0
                ...
                if TargetVisitor.objects.filter(assigned_to=Emp_id).exists():
                    target_data = TargetVisitor.objects.filter(assigned_to=Emp_id).first()
                    total = int(target_data.monthly_target)
                    visit = int(target_data.visited)
                    left = total- visit
                    if left<0:
                        left = 0
                context = {
            ...
            # rep_data = Employee.objects.filter(reportingTo=Emp_id).values("SalesEmployeeCode")
            # if filter_by == "Daily":
            #     # if DailyCustomerService.objects.filter(ase_id=Emp_id).exists():
            #     daily_data = DailyCustomerService.objects.filter(ase_id__in=rep_data, create_date=tdate).count()
            #     target_data = TargetVisitor.objects.filter(assigned_to__in=rep_data)
            #     for data in target_data:
            #         total = total+int(data.daily_visit)
            #         visit = visit+daily_data
            #         left  = total - visit
            #         if left < 0:
            #             left = 0
            #     context = {
