...
from .serializers import BranchSerializer, SettingBranchSerializer, SettingGetBranchSerializer
...
#Setting Branch All API
@api_view(["GET"])
def all_branch(request):
    Branchs_obj = SettingBranch.objects.all() 
    Branch_json = SettingGetBranchSerializer(Branchs_obj, many=True)
    return Response({"message": "Success","status": 200,"data":Branch_json.data})

#Setting Branch One API
@api_view(["POST"])
def one_branch(request):
    id=request.data['id']
...
def create_branch(request):
    data = SettingBranchSerializer(data=request.data)
    if data.is_valid():
        data.save()
        return Response({"message":"successful","status":"200","data":[]})
    return Response({"message":"failed","status":"404","data":[]})
...
def update_branch(request):
    id= request.data["id"]
    obj = SettingBranch.objects.filter(id=id).first()
    data = SettingBranchSerializer(obj, data=request.data)
    if data.is_valid():
        data.save()
        return Response({"message":"successful","status":"200","data":[]})
    return Response({"message":"failed","status":"404","data":[]})
