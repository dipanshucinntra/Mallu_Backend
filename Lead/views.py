...
def create(request):
    ...
    try:
        ...
        ########################## NEW KEYS ####################
        AttendedBy = leads['AttendedBy']
        VisitDate = leads['VisitDate']
        ContactTwo = leads['ContactTwo']
        FunctionDate = leads['FunctionDate']
        FunctionType = leads['FunctionType']
        Venue = leads['Venue']
        ...
        if (phoneNumber=="" or companyName=="" ): ##### or contactPerson==""
            return Response({"message":"Mobile or Person or Company required","status":"201","data":[]})            
        ...
        #     log.append("This Company already exists: "+str(companyName))
        elif Lead.objects.filter(companyName=companyName, contactPerson=contactPerson, phoneNumber=phoneNumber).exclude(companyName="n/a").exists():
            print("This ContactPerson already exists: "+contactPerson)
            log.append("This ContactPerson already exists: "+contactPerson)
        else:
            model=Lead(date=date, DivCode=DivCode, DivName=DivName, tender=tender, location=location, 
                       companyName=leads['companyName'], numOfEmployee=numOfEmployee, turnover=turnover, 
                       source=source, source_id=source_id, contactPerson=contactPerson, designation=designation, 
                       phoneNumber=phoneNumber, message=message, email=email, status=status, leadType=leadType, 
                       productInterest=productInterest, assignedTo_id=assignedTo_id, employeeId_id=employeeId_id, 
                       timestamp=timestamp, category = category,groupType = groupType,intProdCat = intProdCat,
            ...
            # print("LeadID ID:", LeadID.id, "LeadID Id Type : ", type(LeadID.id))         
            if request.data['status'] == "Qualified":
                Subject = request.data['companyName']
                Comment = request.data['Venue']
                CreateDate =str(str(datetime.now()))
                CreateTime = str(datetime.today().strftime("%I:%M %p"))
                Emp = request.data['employeeId'] #Employee Id
                From = request.data['FunctionDate']
                To = request.data['FunctionDate']
                SourceID = str(LeadID.id)
                SourceType = "Lead"
                Time =str(datetime.today().strftime("%I:%M %p"))
                Type= "Followup"
                leadType = ""  ########### This will be blank
                CardName = request.data['companyName']                    
                EmailAddress = request.data['email']
                Phone1 = request.data['phoneNumber']                                      
                CreatedBy=request.data['employeeId']
                ########################## NEW KEYS ####################
                AttendedBy = request.data['AttendedBy']
                VisitDate = request.data['VisitDate']
                ContactTwo = request.data['ContactTwo']
                FunctionDate = request.data['FunctionDate']
                FunctionType = request.data['FunctionType']
...
def mark_junk(request):
    ...
    junk_timestamp= datetime.now() #request.data['junk_timestamp'] 2023-09-14 3:35:45 PM
    # print(datetime.now())
    junk_by=request.data['junk_by']
    naid = []
    for fetchid in fetchids:
        print(fetchid)
        if Lead.objects.filter(pk=int(fetchid)).exists():
           fetchdata=Lead.objects.filter(pk=fetchid).update(junk=status, junk_timestamp=junk_timestamp, junk_by=junk_by)
        else:
            naid.append(fetchid)
    print(str(naid))
...
def all_lead(request):
    leads_obj = Lead.objects.all().values("id", "companyName")     
    serializers = LeadSerializer(leads_obj, many=True).data
...
# {"leadType": "All", "assignedTo": "1"}
#Lead All Filter API
@api_view(["POST"])
def all_filter(request):
    json_data = request.data
    if len(json_data)==1:   #for pagination
        ...
        if len(json_data) == 0:
            ...
            print('by filter employee details')
        ...
        if len(json_data) == 0:
            ...
            #     elif ke =='employeeId' :
            #         if json_data['employeeId'] !='':
            #             leads_obj = Lead.objects.filter(junk=0,employeeId=json_data['employeeId']).order_by("-id")[startWith:endWith]
...
#             SalesPersonID = [json_data['assignedTo']]
#     if len(json_data) == 0:
#         leads_obj = Lead.objects.all(junk=0).count()
#         return Response({"message": "Success","status": 200,"data":[{"lead_count":leads_obj}]})
#     else:
#         if json_data['assignedTo'] !='':
#             leads_obj = Lead.objects.filter(Q(junk=0, companyName__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=0, phoneNumber__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=0, contactPerson__icontains=SearchText, assignedTo__in=SalesPersonID)).count()
#             if leads_obj ==0:
...
#         allld = []
#         if json_data['assignedTo'] !='':
#             leads_obj = Lead.objects.filter(Q(junk=0, companyName__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=0, phoneNumber__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=0, contactPerson__icontains=SearchText, assignedTo__in=SalesPersonID)).order_by("-id")[startWith:endWith]
#             print(leads_obj)
#             if len(leads_obj) ==0:
#                 return Response({"message": "Success","status": 200,"data":[]})
#             else:
...
def count_all(request):
    ...
    try:
        ...
        Zone = ""
    try:
        AssignedUser = json_data['AssignedUser']
    except:
        AssignedUser = ""
    try:
        AssignedDate = json_data['AssignedDate']
    except:
        AssignedDate = ""
    if json_data['assignedTo']!="":
        SalesPersonID = json_data['assignedTo']
    ...
    if len(json_data) == 0:
        ...
        if json_data['assignedTo'] !='':
            leads_obj = Lead.objects.filter(Q(junk=0, companyName__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=0, phoneNumber__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=0, contactPerson__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=0, id__icontains=SearchText, assignedTo__in=SalesPersonID))
            if FromDate != "":
                leads_obj = leads_obj.filter(date__gte=FromDate)
            if ToDate != "":
                leads_obj = leads_obj.filter(date__lte=ToDate)
            if Source != "":
                leads_obj = leads_obj.filter(source_id=Source)
            if GroupType != "":
                leads_obj = leads_obj.filter(groupType=GroupType)
            if States != "":
                leads_obj = leads_obj.filter(state_code=States)
            if Status != "":
                leads_obj = leads_obj.filter(status=Status)
            if Zone != "":
                leads_obj = leads_obj.filter(location=Zone)
            if AssignedUser != "":
                leads_obj = leads_obj.filter(assignedTo=AssignedUser)
            if AssignedDate != "":
                leads_obj = leads_obj.filter(AssignedDate=AssignedDate)
            lead_obj = leads_obj.count()
            return Response({"message": "Success","status": 200,"data":[{"lead_count":lead_obj}]})
...
def all_filter_page(request):
    ...
    if json_data['assignedTo']!="":
        ...
        # elif emp_obj.role == 'admin' or emp_obj.role == 'ceo' or emp_obj.role == "marketing" or emp_obj.role == "Service Head" or emp_obj.role == "HO":
        #     emps = Employee.objects.filter(id__gt=0)
        #     SalesPersonID=[]
        #     for emp in emps:
        #         SalesPersonID.append(emp.id)
    ...
    if len(json_data) == 0:
        ...
        if json_data['assignedTo'] !='':
            # leads_obj = Lead.objects.filter(Q(junk=0, companyName__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=0, phoneNumber__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=0, contactPerson__icontains=SearchText, assignedTo__in=SalesPersonID)).order_by("-id")[startWith:endWith]
            # print(leads_obj)
            leads_obj = Lead.objects.filter(Q(junk=0, companyName__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=0, phoneNumber__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=0, contactPerson__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=0, id__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=0, code__icontains=SearchText, assignedTo__in=SalesPersonID))
            print(leads_obj)
            if FromDate != "":
                leads_obj = leads_obj.filter(date__gte=FromDate)
            if ToDate != "":
            ...
            if len(leads_obj) ==0:
                return Response({"message": "Success","status": 200,"data":[], "total_count":total_count})
            else:
                # allld = []
                for obj in leads_obj:
                    lead_json = LeadSerializer(obj, many=False)
                    ...
                    #added by millan on 22-September-2022
                    allld.append(lead_json_dump)
                return Response({"message": "Success","status": 200,"data":allld, "total_count":total_count})
...
def all_filter_junk(request):
    ...
    if len(json_data) == 0:
        ...
        leads_obj = Lead.objects.filter(Q(junk=1, companyName__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=1, phoneNumber__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=1, contactPerson__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=1, id__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=1, code__icontains=SearchText, assignedTo__in=SalesPersonID))
        if Source != "":
            leads_obj = leads_obj.filter(source_id=Source)
        #leads_json = LeadSerializer(leads_obj, many=True)
        allld = []
        for obj in leads_obj:
            ...
            item_json = LeadItemSerializer(items, many=True)
            if obj.junk_by: 
                emp_name = Employee.objects.filter(SalesEmployeeCode=obj.junk_by).first()
                lead_json_dump['junk_by'] = emp_name.SalesEmployeeName
            lead_json_dump['LeadItem'] = item_json.data
            allld.append(lead_json_dump)
        return Response({"message": "Success","status": 200,"data":allld})
        #return Response({"message": "Success","status": 200,"data":leads_json.data})
        ...
        leads_obj = Lead.objects.filter(Q(junk=1, companyName__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=1, phoneNumber__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=1, contactPerson__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=1, id__icontains=SearchText, assignedTo__in=SalesPersonID)|Q(junk=1, code__icontains=SearchText, assignedTo__in=SalesPersonID))
        if FromDate != "":
            leads_obj = leads_obj.filter(junk_timestamp__gte=FromDate+" 23:59:59")
        if ToDate != "":
            leads_obj = leads_obj.filter(junk_timestamp__lte=ToDate+" 23:59:59")
        if Source != "":
            leads_obj = leads_obj.filter(source_id=Source)
        if GroupType != "":
            leads_obj = leads_obj.filter(groupType=GroupType)
        ...
        if AssignedDate != "":
            leads_obj = leads_obj.filter(AssignedDate=AssignedDate)
        if junk_by != "":
            leads_obj = leads_obj.filter(junk_by=junk_by)
        ...
        if leads_count ==0:
            return Response({"message": "Success","status": 200,"data":[], "Extra":{"total_count":leads_count}})
        else:
            #leads_json = LeadSerializer(leads_obj, many=True)
            allld = []
            for obj in leads_obj:
...
def update(request):
    ...
    try:
        if (request.data['phoneNumber']=="" or request.data['companyName']=="" ): # or request.data['contactPerson']=="" Remove as per Rahul Instruction
            return Response({"message":"Mobile or Person or Company required","status":"201","data":[]})
        if Lead.objects.filter(phoneNumber=request.data['phoneNumber']).exclude(pk=fetchid).exists():
        ...
        elif Lead.objects.filter(companyName=request.data['companyName'], contactPerson=request.data['contactPerson'], phoneNumber=request.data['phoneNumber']).exclude(pk=fetchid).exclude(companyName="n/a").exists():
            ...
            if model.status !="Qualified":
                ...
                model.VisitDate = request.data['VisitDate']
                model.ContactTwo = request.data['ContactTwo']
                model.FunctionDate = request.data['FunctionDate']
                model.FunctionType = request.data['FunctionType']
                ...
                return Response({"message":"Already Qualified","status":201,"data":[]})
    except Exception as e:
        return Response({"message":str(e),"status":201,"data":[]})
    ...
    try:
        ...
        for fetchid in fetchids:
            if Lead.objects.filter(pk=fetchid).exists():
                model= Lead.objects.get(pk=fetchid)
                if emp.role == "ASE":
                    if model.groupType != "Dealer":
                        model.assignedTo = emp
                        model.AssignedDate = date.today()
                        model.save()
                    else:
                        name = model.companyName
                        lead_name.append(name)
                else:
                    model.assignedTo = emp
                    model.AssignedDate = date.today()
                    model.save()
                ...
                naid.append(fetchid)
        if len(lead_name) == 0:
            lead_name = "successful"
        return Response({"message":str(lead_name),"status":200,"data":[]})
    except Exception as e:
...
def lead_dashboard(request):
    ...
    if "SalesEmployeeCode" in json_data:
        ...
        if json_data['SalesEmployeeCode']!="":
            ...
            #------Groupwise-------
            # ["Customer","Prospect ","Distibutor","Dealar","Other"]
            Customer = Lead.objects.filter(assignedTo__in=emp_ids, groupType='Customer').count()            
            Prospect = Lead.objects.filter(assignedTo__in=emp_ids, groupType='Prospect').count()            
            Distibutor = Lead.objects.filter(assignedTo__in=emp_ids, groupType='Distibutor').count()            
            Dealar = Lead.objects.filter(assignedTo__in=emp_ids, groupType='Dealar').count()            
            Other = Lead.objects.filter(assignedTo__in=emp_ids, groupType='Other').count()
            ...
            context = {
                ...
                "Campaigns":camp_all,
                "Converted_Opportunities":con_opp
            }
...
def create_by_excel(request):
    ...
    try:
        ...
        for lead in leads:
            ...
            VisitDate = lead['AttendedBy']
            ContactTwo = lead['ContactTwo']
            FunctionDate = lead['FunctionDate']
            FunctionType = lead['FunctionType']
            Venue = lead['Venue']
            Reference = lead['Reference']
            NumberVisits= lead['NumberVisits']            
            ...
            #     log.append("This Company already exists: "+str(companyName))
                ...
                model=Lead(date=date, DivCode=DivCode, DivName=DivName, tender=tender, location=location, 
                        companyName=companyName, numOfEmployee=numOfEmployee, turnover=turnover, source=source, 
                        source_id=source_id, contactPerson=contactPerson, designation=designation, phoneNumber=phoneNumber, 
                        message=message, email=email, status=status, leadType=leadType, productInterest=productInterest, assignedTo_id=assignedTo_id,
                        employeeId_id=employeeId_id, timestamp=timestamp, category = category,groupType = groupType,intProdCat = intProdCat,
                        intProjCat = intProjCat,country = country,country_code = country_code,state = state,
                        state_code = state_code,city = city, AttendedBy =AttendedBy, VisitDate = VisitDate, ContactTwo = ContactTwo, FunctionDate = FunctionDate,
                        FunctionType = FunctionType, Venue = Venue, Reference= Reference, NumberVisits = NumberVisits)
                model.save()
                print('obj',model)
...
def all_pagination(request):
    try:
        ...
        if json_data['assignedTo'] !="":
            SalesPersonID = json_data['assignedTo']
            SalesPersonID = showEmployeeData(SalesPersonID)
            leads_obj = leads_obj.filter(assignedTo__in=SalesPersonID)
        if json_data['FromDate'] !="":
            leads_obj = leads_obj.filter(date__gte=json_data['FromDate'])
        if json_data['ToDate'] !="":
            leads_obj = leads_obj.filter(date__lte=json_data['ToDate'])
        ...
        if json_data['Status'] !="":
            leads_obj = leads_obj.filter(status=json_data['Status'])
        if json_data['GroupType'] !="":
            leads_obj = leads_obj.filter(groupType=json_data['GroupType']) 
        if json_data['States'] !="":
            leads_obj = leads_obj.filter(state_code=json_data['States'])
        if json_data['Zone'] !="":
            leads_obj = leads_obj.filter(location=json_data['Zone'])
        if json_data['AssignedUser'] !="":
            leads_obj = leads_obj.filter(assignedTo=json_data['AssignedUser'])
        if json_data['AssignedDate'] !="":
            leads_obj = leads_obj.filter(AssignedDate=json_data['AssignedDate'])
        if SearchText !="":
            leads_obj = leads_obj.filter(Q(companyName__icontains=SearchText)|Q(phoneNumber__icontains=SearchText)|Q(leadType__icontains=SearchText)|Q(email__icontains=SearchText)|Q(AttendedBy__icontains=SearchText))

        count = leads_obj.count()
        if MaxItem != "All":
            endWith = (PageNo * int(MaxItem))
            startWith = (endWith - int(MaxItem)) 
            leads_obj = leads_obj[startWith:endWith]                
        serializer = LeadSerializer(leads_obj, many=True)
...
@api_view(["POST"])
def all_pagination_junk(request):
    try:
        json_data = request.data
        PageNo = json_data['PageNo']
        SearchText = json_data['SearchText']
