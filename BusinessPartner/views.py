...
def create(request):
    try:
        ...
        if BusinessPartner.objects.filter(CardName=request.data['CardName'], Phone1=request.data['Phone1']).exists():
            ...
            try:
                ...
                On_Board_Date = request.data['On_Board_Date']
                if U_TYPE == "Dealer":
                    bp_data = BusinessPartner.objects.filter(U_TYPE="Dealer").count()+1
                    Username = "DX"+str(request.data['BPAddresses'][0]['State'])+str(format(bp_data, '04'))
                    Password = request.data['Password']
                else:
                    Username = ""
                    Password = ""
                # CustomerStatus=request.data['CustomerStatus']
...
def all_filter_page(request):
    ...
    allbp = [];
    businesspartners_obj = BusinessPartner.objects.filter(Q(CardName__icontains=search)|Q(EmailAddress__icontains=search)|Q(Phone1__icontains=search)).order_by("-id")
    count = businesspartners_obj.count()
    for bp in businesspartners_obj:
        # Business Partner details obj
        bpObj = BusinessPartner.objects.get(pk=bp.id)
        bp_json = BusinessPartnerSerializer(bpObj)
    ...
    return Response({"message": "Success","status": 200,"data":allbp, "total_count":count})
...
def count_all(request):
    search = request.data['SearchText']
    print(search)
    businesspartners_obj = BusinessPartner.objects.filter(Q(CardName__icontains=search)|Q(EmailAddress__icontains=search)|Q(Phone1__icontains=search)).count() 
    print(businesspartners_obj)
    return Response({"message": "Success","status": 200,"data":[{"total_count":businesspartners_obj}]})
...
def all_filter_page_web(request):
    ...
    allbp = [];
    emp_list = showEmployeeData(Emp_id)
    businesspartners_obj = BusinessPartner.objects.filter(Q(SalesPersonCode__in=emp_list, CardName__icontains=search)|Q(SalesPersonCode__in=emp_list, EmailAddress__icontains=search)|Q(SalesPersonCode__in=emp_list, Phone1__icontains=search)).order_by("-id")
...
def update(request):
    try:
        ...
        if BusinessPartner.objects.filter(CardName=request.data['CardName'], Phone1=request.data['Phone1']).exclude(id=request.data['id']).exists():
            return Response({"message":"Already exist Card Name","status":"409","data":[]})
        else:
            ...
            model.PAN_Card=request.data['PAN_Card']
            model.Adhaar_Number=request.data['Adhaar_Number']

            # model.CustomerStatus=request.data['CustomerStatus']
...
def target_anu_ach(request):
    try:
        ...
        SalesPersonCode_arr = showEmployeeData(SalesPersonCode)
        print("SalesPersonCode")
        print(SalesPersonCode_arr)
        SalesPersonCode_list = ",".join(SalesPersonCode_arr)
        print(SalesPersonCode_list)
...
def customer_status(request): 
    try:
        ...
        cust_obj = BusinessPartner.objects.filter(id=cust_id, U_TYPE="Dealer").update(Is_Active=status)
...
def all_pagination(request):
    ...
    if Emp_id !="":
        emp_list = showEmployeeData(Emp_id)
        businesspartners_obj = BusinessPartner.objects.filter(Q(SalesPersonCode__in=emp_list, CardName__icontains=search)|Q(SalesPersonCode__in=emp_list, EmailAddress__icontains=search)|Q(SalesPersonCode__in=emp_list, Phone1__icontains=search)|Q(SalesPersonCode__in=emp_list, AttendedBy__icontains=search)|Q(SalesPersonCode__in=emp_list, Venue__icontains=search)).order_by("-id")
    else:
        businesspartners_obj = BusinessPartner.objects.filter(Q(CardName__icontains=search)|Q(EmailAddress__icontains=search)|Q(Phone1__icontains=search)|Q( AttendedBy__icontains=search)|Q(Venue__icontains=search)).order_by("-id")    
